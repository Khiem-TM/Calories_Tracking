...
function ExerciseRow({ ex, onSelect }: { ex: Exercise; onSelect: (e: Exercise) => void }) {
  return (
    <button
      key={ex.id}
      onClick={() => onSelect(ex)}
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {ex.description && (
          <p className="text-xs text-slate-400 truncate mt-0.5">{ex.description}</p>
        )}
      </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="max-h-72 overflow-y-auto -mx-2 px-2">
            ...
            {!isLoading && exercises.map((ex) => (
              <ExerciseRow key={ex.id} ex={ex} onSelect={setSelectedExercise} />
            ))}
