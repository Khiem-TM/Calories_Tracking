...
async function testFlow() {
  try {
    ...
    form.append(
      "file",
      fs.createReadStream("/Users/UET/DACN/Project/sample/ok.jpg")
    );
    form.append("photoType", "front");

    const uploadRes = await axios.post(
      `${API_URL}/body-metrics/photos`,
      form,
      {
        headers: {
          ...form.getHeaders(),
    ...
    const photoUrl =
      photosRes.data?.data?.[0]?.photoUrl || uploadRes.data?.data?.photoUrl;
    ...
    console.error("Error during test flow:", err.response?.data || err.message);
  }
}
