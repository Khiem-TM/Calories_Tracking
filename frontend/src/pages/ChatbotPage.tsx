...
export default function ChatbotPage() {
  ...
  const { data: sessions } = useQuery({
    ...
    queryFn: () => chatbotService.getSessions().then((r) => r.data?.data ?? r.data),
  })

  const { data: activeSession } = useQuery({
    queryKey: ['chatbot-session', activeSessionId],
    queryFn: () => chatbotService.getSession(activeSessionId!).then((r) => r.data?.data ?? r.data),
  ...
  const { mutate: sendMessage, isPending: sending } = useMutation({
    mutationFn: (message: string) => chatbotService.sendMessage(activeSessionId!, message),
    onSuccess: () => {
    ...
    <>
      <div className="topbar">
        ...
        <div className="topbar-right">
          <button className="btn btn-ghost2 btn-sm" onClick={() => setActiveSessionId(null)}>+ Cuộc trò chuyện mới</button>
        </div>
      </div>
      ...
      <div className="chat-layout">
        ...
        <div className="chat-history hidden md:flex">
          <div className="chat-history-header">
            ...
            <button className="new-chat-btn" onClick={() => setActiveSessionId(null)} title="Cuộc trò chuyện mới">+</button>
          </div>
          ...
          <div className="history-list">
            ...
            {sessionList.map((s) => (
              ...
              >
                <div className="history-title">{/* Optional: compute title from first message */}Cuộc trò chuyện {new Date(s.createdAt).toLocaleDateString()}</div>
                <div className="history-meta">
                  <span>{new Date(s.createdAt).toLocaleDateString()}</span>
                  <span style={{ marginLeft: 'auto', cursor: 'pointer' }} onClick={(e) => { e.stopPropagation(); deleteSession(s.id); }}>Huỷ</span>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Chat main */}
        <div className="chat-main" style={{ borderRadius: '16px', overflow: 'hidden', border: '1px solid var(--border)', marginRight: '24px' }}>
          <div className="chat-header">
            <div className="ai-avatar">🤖</div>
            <div>
              <div className="ai-name">Tracker AI — Chuyên gia Dinh dưỡng & Thể thao</div>
          ...
          <div className="messages" id="messages">
              ...
              <div className="msg ai">
                ...
                <div>
                  <div className="msg-bubble">
                    Xin chào! Tôi là trợ lý AI của Tracker, được huấn luyện bởi đội ngũ chuyên gia dinh dưỡng và thể thao hàng đầu.<br/><br/>
                    Tôi có thể giúp bạn:
            ...
            {messages.map((msg) => (
              <div key={msg.id} className={`msg ${msg.role === 'user' ? 'user' : 'ai'}`}>
                <div className="msg-avatar">{msg.role === 'user' ? 'TM' : '🤖'}</div>
                <div>
          ...
          {!activeSessionId && (
            <div className="suggestion-chips">
              <div className="chip" onClick={() => setInput('Phân tích báo cáo tuần này')}>Phân tích báo cáo tuần này</div>
              <div className="chip" onClick={() => setInput('Lịch tập tuần tới cho tôi')}>Lịch tập tuần tới cho tôi</div>
              <div className="chip" onClick={() => setInput('Công thức bữa sáng giàu protein')}>Công thức bữa sáng giàu protein</div>
              <div className="chip" onClick={() => setInput('Tôi có nên dùng creatine?')}>Tôi có nên dùng creatine?</div>
            </div>
          )}
          ...
          <div className="chat-input-area border-t !border-[#c9e4d4]">
            <div className="input-box">
              <div className="input-actions">
                ...
                <button className="input-action-btn" title="Gửi ảnh bữa ăn">📷</button>
              </div>
              <textarea 
                value={input}
                ...
                style={{ height: 'auto' }}
              />
              <button className="send-btn" onClick={handleSend} disabled={!input.trim() || sending || creating}>
                <svg width="16" height="16" fill="none" viewBox="0 0 16 16"><path d="M2 8l12-6-6 12-2-4-4-2z" fill="white"/></svg>
              </button>
            </div>
            <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 8, textAlign: 'center' }}>
              Tracker AI có thể mắc lỗi. Tham khảo chuyên gia y tế cho quyết định quan trọng.
            </div>
          </div>
        </div>
      </div>
